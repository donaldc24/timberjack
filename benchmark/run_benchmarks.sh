...
run_json_benchmarks() {
    ...
    for size in "10k" "100k" "1m"; do
        ...
        # Python json parser benchmark
        benchmark "json" "python-json" "python3 -c \"import json, sys; print(len([x for x in (json.loads(l) for l in open('$json_file')) if x.get('level')=='ERROR']))\"" "$json_file"
    done
}
...
run_stats_benchmarks() {
    ...
    for size in "10k" "100k" "1m"; do
        ...
        # No good comparison tools for stats - this is a unique Timberjack feature
        # But we can compare to a custom Python script for similar functionality
        benchmark "stats" "python-stats" "python3 -c \"
...
analyze_text_log('$log_file')
\" > /dev/null" "$log_file"

        benchmark "stats" "python-json-stats" "python3 -c \"
...
generate_reports() {
    ...
    cat > $BENCH_DIR/reports/$TIMESTAMP/analyze_benchmarks.py << 'EOF'
import os
import sys
import pandas as pd
